...
class SimProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        file_path = os.path.join(data_dir, 'comment_train.csv')
...
class BertSim:
    ...
    def __init__(self, batch_size=args.batch_size):
        self.mode = None
        self.max_seq_length = args.T_max_seq_len
    ...
    def get_estimator(self):
        ...
        return Estimator(model_fn=model_fn,
                         config=RunConfig(session_config=config,save_summary_steps=1000,
                         save_checkpoints_steps = 1000,keep_checkpoint_max = 1),
    ...
    def generate_from_queue(self):
        while True:
            ...
            featureses = list(self.convert_examples_to_features(predict_examples, self.processor.get_labels(),
                                                              args.T_max_seq_len, self.tokenizer))
            #判断多个输入同时输出
            if len(featureses)>1:
                for featuress in featureses:
                    features=[featuress]
                    yield {
            ...
            #单个文本预测
                ...
                yield {
                    'input_ids': [f.input_ids for f in featureses],
                    'input_mask': [f.input_mask for f in featureses],
                    'segment_ids': [f.segment_ids for f in featureses],
                    'label_ids': [f.label_id for f in featureses]
                }
    ...
    def train(self):
        ...
        if args.T_max_seq_len > bert_config.max_position_embeddings:
            raise ValueError(
                "Cannot use sequence length %d because the BERT model "
                "was only trained up to sequence length %d" %
                (args.T_max_seq_len, bert_config.max_position_embeddings))
        ...
        self.file_based_convert_examples_to_features(train_examples, label_list, args.T_max_seq_len, self.tokenizer,
                                                     train_file)
        tf.logging.info("***** Running training *****")
        tf.logging.info("  Num examples = %d", len(train_examples))
        tf.logging.info("  Batch size = %d", args.batch_size)
        tf.logging.info("  Num steps = %d", num_train_steps)
        train_input_fn = self.file_based_input_fn_builder(input_file=train_file, seq_length=args.T_max_seq_len,
                                                          is_training=True,
                                                          drop_remainder=True)
        #提前终止，作用相当于正则化
    ...
    def eval(self):
        ...
        self.file_based_convert_examples_to_features(
            eval_examples, label_list, args.T_max_seq_len, self.tokenizer, eval_file)
    ...
    def predict_one(self, sentence1, sentence2):
        if self.mode is None:
            raise ValueError("Please set the 'mode' parameter")
    ...
    def predict_sentences(self, sentences1, sentences2):
        if self.mode is None:
            raise ValueError("Please set the 'mode' parameter")
        #改造成predict_class
        sentence=list(zip(sentences1,sentences2))
        self.input_queue.put(sentence)
        prediction=[]
        for _ in range(len(sentence)):
            prediction.append(self.output_queue.get())
        return prediction
...
if __name__ == '__main__':
    ...
    while True:
        ...
        predict = sim.predict_one(sentence1, sentence2)
        print(f'similarity：{predict[0][1]}')
    # sentences1=['您好','参与打卡']
    # sentences2=['你好','退出打卡']
    # predict = sim.predict_sentences(sentences1, sentences2)
    # print(predict)